...
export function ReviewList() {
   ...
   const getSubjectsDisplay = () => {
      ...
      if (subjectsFilter.length === 1)
         return getSubjectLabelVi(subjectsFilter[0]);
      return `${subjectsFilter.length} môn học`;
   };

   const getLevelsDisplay = () => {
      if (levelsFilter.length === 0) return "Tất cả cấp độ";
      if (levelsFilter.length === 1) return getLevelLabelVi(levelsFilter[0]);
         ...
         <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            ...
            <div className="mb-8">
               ...
               <p className="mt-2 text-pretty text-muted-foreground">
                  Quản lý và theo dõi tất cả đánh giá từ học sinh
               </p>
            </div>
                  ...
                  <div className="flex flex-1 flex-wrap items-center gap-4">
                     ...
                     <Popover>
                        <PopoverTrigger asChild>
                           <Button
                              variant="outline"
                              className="w-[180px] h-12 justify-start text-base"
                           >
                                 ...
                                 {availableSubjects.map((subject) => (
                                    ...
                                    >
                                       <input
                                          type="checkbox"
                                          id={`subject-${subject}`}
                                          checked={subjectsFilter.includes(
                                             subject
                                          )}
                                          onChange={(e) => {
                                             if (e.target.checked) {
                     ...
                     <div className="w-[100px]">
                        <Button
                           ...
                           className={`flex items-center gap-2 whitespace-nowrap transition-opacity duration-200 h-12 text-base ${
                              hasActiveFilters
                  ...
                  <div className="mt-6 flex flex-wrap gap-3">
                     ...
                     {tempFilters.subjectsFilter.length > 0 && (
                        <span className="inline-flex items-center rounded-full bg-purple-100 px-4 py-2 text-base text-purple-800">
                           Môn học:{" "}
                           {tempFilters.subjectsFilter
                              .map((s) => getSubjectLabelVi(s))
                              .join(", ")}
                        </span>
                     )}
                     {tempFilters.levelsFilter.length > 0 && (
                        <span className="inline-flex items-center rounded-full bg-orange-100 px-4 py-2 text-base text-orange-800">
                           Cấp độ:{" "}
                           {tempFilters.levelsFilter
                              .map((l) => getLevelLabelVi(l))
                              .join(", ")}
                        </span>
                     )}
                  </div>
