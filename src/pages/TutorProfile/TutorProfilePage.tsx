...
} from "@/components/tutor/tutor-profile";
...
export default function TutorProfile() {
   ...
   useEffect(() => {
      if (tutorProfile) {
         const user =
            typeof tutorProfile.userId === "object"
               ? tutorProfile.userId
               : null;
         ...
         setFormData((prev) => ({
            ...
            address: {
               city:
                  prev.address?.city ||
                  user?.address?.city ||
                  tutorProfile.address?.city ||
                  "",
               street:
                  prev.address?.street ||
                  user?.address?.street ||
                  tutorProfile.address?.street ||
                  "",
            },
         }));
   ...
   const handleSave = async () => {
      ...
      const validation = validateForm(
         submissionData as TutorProfileFormData,
         !!tutorProfile
      );
   ...
   const handleClassTypeChange = (type: string, checked: boolean) => {
      setFormData((prev) => {
         ...
         setTimeout(
            () => validateField("classType", newClassType, !!tutorProfile),
            0
         );
   ...
   const handleCertificationChange = (
      ...
      if (value && value.trim() !== "") {
         validateField(
            `certifications.${index}.${String(field)}`,
            value,
            !!tutorProfile
         );
      } else {
   ...
   const handleEducationChange = (
      ...
      setTimeout(
         () => validateField("education", newEducation, !!tutorProfile),
         0
      );
   };
   ...
   const handleCertificationImageUpload = (
      certIndex: number,
      files: FileList | null
   ) => {
      if (!files) return;
   ...
   const removeCertificationImage = (certIndex: number, fileIndex: number) => {
      setCertificationFiles((prev) => {
         ...
         if (updatedFiles[certIndex]) {
            updatedFiles[certIndex] = updatedFiles[certIndex].filter(
               (_, i) => i !== fileIndex
            );
   ...
   const handleRemoveExistingImage = (
      ...
      setRemovedImages((prev) => {
         ...
         console.log(
            "📋 Trạng thái removedImages đã cập nhật:",
            newRemovedImages
         );
      ...
      setFormData((prev) => {
         const updatedCertifications = prev.certifications?.map((c, idx) => {
            if (idx === certIndex) {
               const currentUrls = Array.isArray(c.imageUrls)
                  ? c.imageUrls
                  : [];
               const updatedImageUrls = currentUrls.filter(
                  (_, i) => i !== imageIndex
               );
         ...
         <div className="w-full min-h-screen bg-background text-foreground">
            <div className="w-full h-full">
               ...
               <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                              ...
                              <div className="space-y-4">
                                 ...
                                 <div>
                                    ...
                                    <div className="mt-2">
                                       ...
                                       />
                                       <label
                                          htmlFor={`certification-images-${index}`}
                                       >
                                          <Button
                                             variant="outline"
                                             size="sm"
                                    ...
                                    {certificationFiles[index] &&
                                       certificationFiles[index].length > 0 && (
                                          <div className="mt-3">
                                             <p className="text-sm text-muted-foreground mb-2">
                                                Selected images:
                                             </p>
                                             <div className="flex flex-wrap gap-2">
                                                {certificationFiles[index].map(
                                                   (file, fileIndex) => (
                                                      <div
                                                         key={fileIndex}
                                                         className="relative"
                  ...
                  {/* Availability */}
                  <Card
                     className="lg:col-span-3 bg-card text-card-foreground"
                  ...
                  >
                     <CardHeader>
                        ...
                        <p className="text-sm text-muted-foreground">
                           Select the days and time slots when you're available
                           to teach
                        </p>
                     </CardHeader>
                     <CardContent>
